  ...
  bookdown::word_document2:
    ...
    pandoc_args: [
      "--csl", "/home/nicola/Documents/work/projects/Phosphor_project/report/spitsbergen/supplement/common/nature-geoscience.csl", "--citation-abbreviations", "/home/nicola/Documents/work/projects/Phosphor_project/report/spitsbergen/supplement/common/abbreviations.json",
       "--reference-doc", "/home/nicola/Documents/work/projects/Phosphor_project/report/spitsbergen/supplement/common/word-style-ref2.docx"
       ]
...
ID     <- read.csv(paste0(data.path, "data/samplelist/SampleIDs.csv"), 
...
all.Fe <- map_df(ls.Fe %>% set_names(), ~read.csv(file = 
                                                    paste0(data.path, 
                                                          "data/Fe_seq/", .x),
                                                  stringsAsFactors = FALSE),
                 .id = "path") %>%
          as_tibble () %>%
          filter_at(vars(ID), any_vars(str_detect(., pattern = "MS"))) %>%
          mutate(phase=factor(
                          case_when(
...
all.CRS <- map_df(ls.CRS %>% set_names(), 
                  ~read.csv(file = paste0(data.path, "data/CRS/", .x),
                                                      stringsAsFactors = FALSE),
                  .id = "path")
...
#-------------------------------
all.TD  <-list(
               read.csv(paste0(data.path, "data/totals/Total_Sum_ed.csv"), 
                        stringsAsFactors=FALSE) %>%
                  as_tibble() %>%
                  group_by(ID) %>%
                  mutate(newID = row_number()),
               read.csv(paste0(data.path, "data/totals/Total_Sum_TE_ed.csv"), 
                        stringsAsFactors=FALSE) %>%
                  as_tibble() %>%
                  group_by(ID) %>%
                  mutate(newID = row_number())) %>%
...
ls.P <- list.files(path = paste0(data.path, "data/P_seq"), pattern = ".xls") %>%
         set_names()
...
all.ICP.P <- map_df(ls.ICP.P %>% set_names(),
                    ~read.csv(file = paste0(data.path, "data/P_seq/ICP/", .x), 
                              stringsAsFactors = FALSE),
                    .id = "path") %>%
              mutate(phase =
                              case_when(
...
#-------------------------------
# Festningen
TOC_fest <-read.csv(paste0(data.path, "data/TOC/", "MS_TOC_sep2016.csv"), 
                    stringsAsFactors=FALSE) %>%
           as_tibble() %>% select(ID = Name, value = Carbon)

# Deltadalen
TOC_dd <-read.csv(paste0(data.path, "data/TOC/", "MS_TOC_march2018.csv"), 
                  stringsAsFactors=FALSE)%>%
           as_tibble() %>% select(section.ID = Name, value = Carbon)

# sample labels TOC
ID_TOC     <- read.csv(paste0(data.path, "data/samplelist/SampleIDs_TOC.csv"), 
                       stringsAsFactors=FALSE) %>% 
              as_tibble()
...
all.TOC <- all.TOC %>% mutate(value = ((1 - ((weight_tube_gram +
                                              weight_samples_gram) -
                                              weight_after_gram) /
...
# loading d13C data
#-------------------------------
#setwd(paste(path, "data/Cisotope", sep=""))
all.d13C   <- read.csv(paste0(data.path, "data/Cisotope/d13C_march2018_ed.csv"),
                       stringsAsFactors=FALSE) %>%
                        as_tibble() %>%
...
# summary stats for d13C QCs
sum_stat_d13C <- read.csv(paste0(data.path,
                                 "data/Cisotope/d13C_Stds_march2018.csv"),
                          stringsAsFactors = FALSE) %>%
                          as_tibble() %>%
                          group_by(ID) %>%
...
# loading d34S data
# setwd(paste(path, "data/Sisotope", sep = ""))
d34S_fest <- read.csv(paste0(data.path, "data/Sisotope/d34S_Fest.csv"),
                      stringsAsFactors = FALSE) %>% as_tibble()
d34S_DD <- read.csv(paste0(data.path, "data/Sisotope/d34S_DD.csv"),
                    stringsAsFactors = FALSE) %>% as_tibble()

# function for name reformatting
stringy<-function(x) {cm<-strsplit(x, split = "-") 
            return(cm[[1]][2])}

# reformatting raw data output (only Festningen)
d34S_fest <- d34S_fest %>% select(SN = contains(".Number"),
                                  Aq.T = contains("date"),
                                  Name,
                                  RT = contains("Sec"),
                                  Amps = contains("..nA"),
...
# create labels for sampling blocks
 d34S_bl <-  d34S_fest %>% distinct(SN, .keep_all = TRUE) %>%
...
# summary stats for d34S QCs
sum_stat_d34S <- d34S_fest %>% filter(section == "standard") %>% 
                 group_by(Name) %>% 
                 summarize(mean = mean(S34), sd = sd(S34), n=n()) %>% 
                 mutate(assigned=c(ls.Chem$as_CP1,NA,ls.Chem$as_S3, ls.Chem$as_sws)) %>% 
...
all.d34S <- bind_rows(d34S_fest %>% 
                      ...
                      d34S_DD) %>% 
            mutate(section = if_else(str_detect(Name, "Fe"), "Festningen", 
                                     "Deltadalen"),
                   phase = "d34S", method = "EA-MS", unit = "\u2030") %>% 
            select(section.ID = Name, section, phase, method, value = S34, unit)
...
# concentration (wt.%), requiring the machine output (ppm), solution volume (ml)
# and sample weight (g).
...
#---------------------------
# storing duplicates
dups.Fe <- all.Fe %>% group_by(phase, section.ID) %>%
...
all.Fe <- all.Fe %>% group_by(phase, section.ID) %>%
...
all.Fe <-  all.CRS %>% 
           ...
           mutate(value = CRSWeight((Weight_final - Weight_filter), 
                                     Weight_gram)) %>% 
           ungroup() %>% 
           right_join( . , all.Fe %>% filter(phase == "Fe_mag") %>% 
                                    select(gl.ID, height), 
                       by = "section.ID") %>% 
           mutate(unit = "%", phase = "Fe_py", method = "CRS") %>% 
           select(gl.ID, height, phase, method, value, unit) %>% 
           bind_rows(all.Fe)
...
AbCalib <- function(Absorb, C_lab, Calib){
  ...
  TA <- c( TA, unname(((lm(ppm~A, 
                           Calib[[which(names(Calib) %in% 
                                          C_lab[i])]]))$coefficients)[2]*
  Absorb[i]+
  ((lm(ppm~A, Calib[[which(names(Calib) %in% C_lab[i])]]))$coefficients)[1]))
...
all.P <- all.P %>% 
         rowwise() %>% 
         mutate(value = EWeight(AbCalib(Absorb = A, C_lab = Calib, 
                                        Calib = CBs.com), "ppm", V, 
                                        weight_gram)) %>%
...
all.P <- all.ICP.P %>%  
         left_join( . , all.P %>% filter(Label == "CDB_MgCl") %>%  
                        select(gl.ID, weight_gram), by = "ID") %>% 
         rowwise() %>% 
         mutate(value = EWeight(value, "ppm", 10, weight_gram)) %>% 
         ungroup() %>%
         bind_rows(. , all.P %>% select(-Label)) %>% 
...
TD.sc <- list(major = c("Fe", "Al", "P"), minor = c("Mo", "U"), ultra = "Re")
...
all.TD <- all.TD %>% 
          ...
          mutate(Output.unit = case_when(phase %in% TD.sc$major ~ "ppm",
...
Loc.coord <- data.frame(name="Sites", x=c(Fest.x, DD.x), y=c(Fest.y, DD.y))
...
```{r FigGeograph,  anchor="figure", fig.cap = "Geographical setting of the Festningen section (1) and Deltadalen core (2). Lithofacies of Spitsbergen after ref @Harland1997."}
...
b <- rasterGrob(img, width = unit(0.3, "npc"), x=  unit(0.85, "npc"), y=  unit(0.78, "npc"))

# assign element to gtable
rect.1  <-  grid.rect(width=unit(0.17, "npc"), height=unit(0.20,"npc"),  x=unit(0.5,"npc"), y=unit(0.66,"npc"), gp=gpar(fill=FALSE, col= "black", ltw=2), draw=FALSE) # black rect in main figure
rect.2  <-  grid.rect(width=unit(0.295, "npc"), height=unit(0.41,"npc"),  x=unit(0.835,"npc"), y=unit(0.775,"npc"), gp=gpar(fill="white", col= "white", ltw=2), draw=FALSE) # white background of inset
rect.3  <-  grid.rect(width=unit(0.298, "npc"), height=unit(0.38,"npc"),  x=unit(0.85,"npc"), y=unit(0.78,"npc"), gp=gpar(fill=FALSE, col= "black", ltw=3), draw=FALSE) # black rect around inset

# creating elements and text to be drawn on map
text.1  <-  grid.text(label= "Panthalassa",  x=unit(0.26,"npc"), y=unit(0.7,"npc"), gp=gpar(fontsize=7), draw=FALSE)
text.2  <-  grid.text(label= "Pangaea",  x=unit(0.95,"npc"), y=unit(0.79,"npc"), rot= 25, gp=gpar(fontsize=7), draw=FALSE)
text.3  <-  grid.text(label= "Barents \n Sea",  x=unit(0.885,"npc"), y=unit(0.845 ,"npc"), rot= -15, gp=gpar(fontsize=5), draw=FALSE)
text.4  <-  grid.text(label= "Sverdrup \n Basin",  x=unit(0.81,"npc"), y=unit(0.83,"npc"), rot=-25, gp=gpar(fontsize=5), draw=FALSE)
text.5  <-  grid.text(label= "Siberian Traps",  x=unit(0.9,"npc"), y=unit(0.94 ,"npc"), rot= 15, gp=gpar(fontsize=5), draw=FALSE)
circ.1 <-  grid.circle(r=unit(0.012, "npc"), x=unit(0.855,"npc"), y=unit(0.805,"npc"), gp=gpar(fill=FALSE, col= "black", ltw=2), draw=FALSE)
...
The Kapp Starostin Formation is dominated by biogenic cherts (spiculites) with subordinate cherty shales, brachiopod and bryozoan-rich limestones and glauconitic sandstones[@Blomeier2013; @Dustira2013, @Bond2015]. Although substantial differences between the lithologies of outer Isfjorden of western-most Spitsbergen (Festningen) and inner Isfjorden of central Spitsbergen (Deltadalen), which has led to the proposal of two distinct members; the Hovtinden and Stensiöfjellet members, respectively, these units grade laterally into each other (Supplementary Fig \@ref(fig:FigLithostrat))[@Blomeier2013] and can therefore be correlated.  These two lithologies are interpreted to record shallow marine, high energy conditions in the inner Isfjorden (Stensiöfjellet Member) whilst dark shales and cherts dominated in more distal, deeper locations (Hovtinden Member) that occurred in the outer Isfjorden[@Blomeier2013; @Bond2017]. Our study section at Festningen comprises the most expanded development of the Kapp Starostin Formation and has relatively thin beds of limestone indicating its distal, deep water development compared to the more easterly Deltadalen site. This sand-rich site was probably part of the inner sandy shelf that developed in north and east Spitsbergen in the Permian (Supplementary Fig \@ref(fig:FigGeograph)).


```{r  FigLithostrat,  anchor="figure", out.width="80%", fig.cap="Detailed lithology logs of the Deltadalen core and Festningen outcrop as well as a correlation scheme linking both sections."}
...
During the Early Triassic the west-to-east thinning of the stratigraphy persisted although the facies became dominated by fine-grained clastic material (of the Vardebukta and  Vikinghøgda formations in the outer and inner Isfjorden, respectively) following the demise of siliceous sponges[@Wignall2016; @Foster2017]. However, glauconitic sandstones persist 1.6$\,$m above the the base of the Vikinghøgda Formation but the lithology is different from the underlying unit as it lacks diagenetic chert nodules (Supplementary Fig \@ref(fig:FigLithostrat))[@Mork1999; @Nabbefeld2010; @Foster2017]. The deposition of these units occurred in an open-marine shelf setting[@Mork1999]. Initially, during the P--Tr boundary interval, deepest-water facies continued to be developed in the Festningen area. Progradation of sandy shoreface facies from a sediment source to the west saw shallower water conditions developed in this area later in the Early Triassic[@Wignall2016]. 

# Chronology

The uppermost beds of the Kapp Starostin Formation are of Late Permian age, where the whole lithological unit (~380$\,$m at Festningen) comprises a long time interval (Artinskian--Changhsingian), based on bryozoa, foraminifera, conodont, and brachiopod biostratigraphy as well as $\delta$^13^C chemostratigraphy[@Nakrem1992; @Dustira2013; @Blomeier2013; @Bond2017]. The abrupt lithological shift at the formation boundary has been interpreted as the die-off of a prolific silica producing metazoan community (sponges)(Supplementary Fig \@ref(fig:FigLithostrat)), and also contains the disappearance of deep-tiered ichno fossil *Zoophycos*, which is synchronous with the global-scale biotic crisis[@Wignall1996]. Although faint signs of life persist in the form of bioturbation in the shale facies up to a height of ~3$\,$m above the base of the Vardebukta Formation at Festningen (Supplementary Fig \@ref(fig:FigLithostrat)), whereafter burrows disappear and sediments become finely laminated. A glauconitic sandstone unit, occurring above the base of the Vikinghøgda Formation (Supplementary Fig \@ref(fig:FigLithostrat)) and contains a diverse ichno fabric assemblage and, as such, the extinction horizon is placed above this unit[@Foster2017]. Occurrences of the ammonite *Otoceras boreale*, the conodont *Clarkina carinata* and species of the bivalve *Claraia*, 5--10$\,$m above the base of the Vardebukta and Vikinghøgda formations are the only age-diagnostic fossils for the Griesbachian (earliest Triassic)[@Harland1997; @Wignall1996; @Mork1999; @Nakrem2008]. Based on these considerations, a latest Permian age has been inferred for the shale and sandstone units above the formational boundary of Festningen and Deltadalen, respectively[@Wignall1996; @Dustira2013; @Nabbefeld2010; @Foster2017]. Carbon isotope based chemostratigraphy offers an alternative, more refined age control, where P--Tr event beds worldwide record a long-term negative $\delta$^13^C anomaly that relates to a perturbation of the global biogeochemical carbon cycle[@Korte2010]. Based on this premise, the $\delta$^13^C~TOC~ minimum has been used as an alternative stratigraphic marker for P--Tr boundary sequences in Spitsbergen, which have so far not yielded the marker conodont species *Hindeodus parvus* that defines the P--Tr boundary worldwide[@Wignall1998; @Dustira2013; @Grasby2015a]. Adopting this approach helped define the position of the P--Tr boundary in both sequences (Fig. 1 and Supplementary Fig \@ref(fig:FigLithostrat)), and suggests that the successions record an age range of roughly Changhsingian to earliest Triassic.

# Materials

The Festningen section was sampled at approximately one meter intervals during a sampling campaign in July 2012. The Deltadalen core was recovered during an expedition in 2014 and the core is currently deposited at the University of Oslo. Samples were taken at a 0.5 meter resolution from the lowest 20 meters of the core. The outcrop samples from Festningen were sawn in pieces and weathered surfaces were discarded along with material showing diagenetic textures. In turn, the samples were crushed manually and, depending on the samples size, either ground with a mechanical agate mill or an agate pestle and mortar.
...
all.CD.long <- list(all.TD,
                    ...
                    all.d34S) %>% 
                      reduce(bind_rows) %>% 
                      select(-height) %>% 
                      left_join(all.TD %>% filter(phase == "Fe") %>%  
...
walk2(df.extr %>% pull(data) , df.extr %>% pull(section), fl.create)            
...
walk2(all.QC.wide %>% pull(data) , all.QC.wide %>% pull(method), fl.create)   
...
all.CD.wide <- all.CD.wide %>% 
               ...
               mutate(Fe_HR_star = 
                      HR_star(X = PRS_FeT, 
                              FeT = Fe, 
                              Fe_HR = Fe_HR, 
                              base = ifelse(section == "Festningen", 
...
# molar Corg over Porg  
               mutate(Corg_Porg  = ifelse(P_org > 0 ,
                                          (TOC / atomicweight$C) / 
                                            (P_org / atomicweight$P), NA),
# molar Corg over Preac                        
                      Corg_Preac = ifelse(HRP_min > 0 ,
                                          (TOC / atomicweight$C) / 
...
#-------------------------------------------
# PAAS element values (Wedepohl, 1971, 1991)
#-------------------------------------------

ls.Chem$PAAS <- data.frame(value = 
                             c(0.15 * 100,   # Mo/Al average shale PAAS (ug/g)
                               0.008,        # P/Al average shale PAAS  (%g/%g) 
                               0.42 * 100,   # U/Al average shale PAAS (ug/g)
                               0.55,         # Fe/Al average shale PAAS (%g/%g)
                               96 * 10^-4,   # Mn/Al average shale PAAS (%g/%g)
                               0.11 * 1000), # Re/Al average shale PAAS (ng/g)
                           phase = c("Mo_Al", "P_Al", "U_Al", "Fe[tot]/Al", 
                                     "Mn_Al", "Re_Al")
                            ) %>% 
                        as_tibble()
#-------------------------------------------
# enrichment factors over PAAS (EF_element X = X/Al_sample/X/Al_average shale; 
# Tribovillard et al., 2006)
#-------------------------------------------

all.CD.wide <- all.CD.wide %>%
# MoEF               
               mutate(Mo_EF = Mo_Al / ls.Chem$PAAS %>% filter(phase == 
                                                                "Mo_Al") %>% 
                        pull(value),
# UEF                     
                      U_EF = U_Al / ls.Chem$PAAS %>% filter(phase == 
                                                                "U_Al") %>% 
                        pull(value),

# ReEF
                      Re_EF = Re_Al / ls.Chem$PAAS %>% filter(phase  == 
                                                                "Re_Al") %>% 
                        pull(value)
                      )

# exclude when P recovery is over 1.3 (mean + 1SD)
#all.CD.wide <- all.CD.wide %>% filter(near(HRP_P_max, mean(HRP_P_max), sd(HRP_P_max)))

#-------------------------------------------
# complete dataset for subsequent processing
#-------------------------------------------

all.CD.long <- all.CD.wide %>% gather(key = phase, value = value, 
                                      -c(section, section.ID, height))

write.csv(all.CD.long, "all.CD.long.csv") 
...
ls.Chem$corr_Feox <- (55.845 * 2 / ((55.845 * 2) + (16 * 3)))
ls.Chem$corr_Pox <- (30.974 * 2 / ((30.974 * 2) + (16 * 5)))
ls.Chem$corr_Alox <- (26.982 * 2 / ((26.982 * 2) + (16 * 3)))

# USGS reference values
# SBC-1
ls.Chem$SBC <- data.frame(phase = c("Fe", "P", "Al", "Mo", "U"),
...
ls.Chem$SGR <- data.frame(phase = c("Fe", "P", "Al", "Mo", "U"),
...
sum.QC.Fe <- all.QC %>% 
          ...
          mutate(phase = recode(phase, !!!good)) %>% 
...
sum.QC.TD <- all.QC %>% 
          ...
          mutate(phase = factor(phase, levels =c("Al", "Fe", "P", "Mo", 
                                                 "U", "Re"),
                                c("Al", "Fe~tot~", "P~tot~", "Mo*", "U*", 
                                  "Re**"),
                                ordered = TRUE
                                )
                 ) %>% 
...
knitr::kable(sum.QC.TD, digits = 2, row.names = FALSE,  format = "pandoc",
  caption = "Summary statistics certified USGS reference material SBC-1 and SGR 1 for major and minor  elements, respectively. Mean, SD and published values are in wt. %, except for elements labelled with an \\*, which are given in $\\mu$g/g, and and \\**, which are given in ng/g")
...
```{r Tabled13C , results='asis', include=FALSE}

#----------------------------------------------------------------------------------------
# tables of summary statistics standards for C isotope analysis
#----------------------------------------------------------------------------------------

knitr::kable(sum_stat_d13C, digits = 1, booktabs = TRUE,
  caption = "Summary statistics $\\delta$^13^C~TOC~ of reference materials, excluding first and last batch of the sequence.")
...
# save images
tiff(paste0(data.path, "report/spitsbergen/figures/SuppFig4.tiff"), height=20, width=20, unit="cm", res=300)

# create gtable
tg <- arrangeGrob(grobs=replicate(4, rectGrob(gp=gpar(col="white", fill="white")), FALSE))
...
# assign element to gtable
rect.1  <-  grid.rect(width=unit(0.07, "npc"), height=unit(0.07,"npc"),  x=unit(0.12,"npc"), y=unit(0.93,"npc"), gp=gpar(fill="white", col= "white"), draw=FALSE)
text.1  <-  grid.text(label= "A",  x=unit(0.12,"npc"), y=unit(0.93,"npc"), gp=gpar(fontsize=7))
text.2  <-  grid.text(label= "B",  x=unit(0.12,"npc"), y=unit(0.93,"npc"), gp=gpar(fontsize=7))
text.3  <-  grid.text(label= "C",  x=unit(0.12,"npc"), y=unit(0.93,"npc"), gp=gpar(fontsize=7))
text.4  <-  grid.text(label= "D",  x=unit(0.12,"npc"), y=unit(0.93,"npc"), gp=gpar(fontsize=7))

tg <- gtable_add_grob(tg, a, 1, 1, z=Inf, name="a")
tg <- gtable_add_grob(tg, b, 1, 2, z=Inf, name="b")
tg <- gtable_add_grob(tg, c, 2, 1, z=Inf, name="c")
tg <- gtable_add_grob(tg, d, 2, 2, z=Inf, name="a")
...
```{r FigPetro, fig.cap = "Microscale mineralogical and elemental variations mapped for A--B) Deltadalen at 95.9$\\,$m depth, and C--D) Festningen 1.6$\\,$m height. Note, that the fine sand-sized clast of Deltadalen are composed of quartz and apatite shell fragments (likely belonging to the inarticulate brachiopods) with porespaces occupied by glauconite and authigenic apatite infillings. Clasts of Festningen are clay-sized, and, although, P is detected with EDS, P bearing minerals could not be identified. The mineral assemblage is dominated by dolomite, ankerite  and  sheet silicates.", out.width="200%"}
...
# save images
tiff(paste0(data.path, "report/spitsbergen/figures/SuppFig5.tiff"), height=20, width=10, unit="cm", res=300)

# create gtable
tg <- arrangeGrob(grobs=replicate(2, rectGrob(gp=gpar(col="white", fill="white")), FALSE))

img <- readPNG(paste0(data.path, "data/paleontology/larval_shell.png"))
a <- rasterGrob(img)

img <- readPNG(paste0(data.path, "data/paleontology/Nucinella.png"))
b <- rasterGrob(img)

# assign element to gtable
rect.1  <-  grid.rect(width=unit(0.07, "npc"), height=unit(0.07,"npc"),  x=unit(0.1,"npc"), y=unit(0.78,"npc"), gp=gpar(fill="white", col= "white"), draw=FALSE)
text.1  <-  grid.text(label= "A",  x=unit(0.1,"npc"), y=unit(0.78,"npc"), gp=gpar(fontsize=7))
text.2  <-  grid.text(label= "B",  x=unit(0.1,"npc"), y=unit(0.78,"npc"), gp=gpar(fontsize=7))

tg <- gtable_add_grob(tg, a, 1, 1, z=Inf, name="a")
tg <- gtable_add_grob(tg, b, 2, 1, z=Inf, name="b")

tg <- gtable_add_grob(tg, rect.1, 1, 1, z=Inf, name="a.lab.rec")
tg <- gtable_add_grob(tg, rect.1, 2, 1, z=Inf, name="b.lab.rec")

tg <- gtable_add_grob(tg, text.1, 1, 1, z=Inf, name="a.lab")
tg <- gtable_add_grob(tg, text.2, 2, 1, z=Inf, name="b.lab")

# draw figures with additional elements
grid.draw(tg)
...
theme_replace(plot.background   = element_rect(fill = "transparent", color = NA),
...
plot.els$FeAl <- list(data.frame(section = c("Festningen", "Deltadalen")) %>% 
                        as_tibble() %>% 
                        mutate(phase = "Fe[tot]/Al" , xmin = 0.44,  xmax = 0.66, 
                               mean =  ls.Chem$PAAS %>% 
                                       filter(phase == "Fe[tot]/Al") %>% 
                                       pull("value")
                                ),
                  data.frame(section = "Festningen", 
                  ...
                  data.frame(section = "Deltadalen", 
                        ...
                        reduce(. , bind_rows) %>% 
                        gather(. , "key", "value", -c(section, 
                                                      phase)) %>% 
                        mutate(linetype = if_else(key == "mean", 1, 0.8)) %>% 
             mutate(phase = factor(phase, levels = c("Fe[tot]/Al", "Fe[PRS]/Fe[tot]"), 
                            ordered = TRUE
                              )
                 )
...
# Festningen
plot.els$PTX_hor_f <- list(geom_hline(yintercept = 3.87), geom_hline(yintercept = 0, linetype=2)) 
# Deltadalen
plot.els$PTX_hor_d <-list(geom_hline(yintercept = 86), geom_hline(yintercept = 87, linetype=2)) 
...
plot.1 <- all.CD.long %>% 
          ...
          mutate(phase = factor(
                            case_when(
                             ...
                             levels = c("Fe[tot]/Al", "Fe[PRS]/Fe[tot]", "P[det]/P[tot]"), 
                             ordered = TRUE
                              )
                 )
...
filter.BB_chron <- BB_chron %>% 
  ...
  mutate(type = factor(type, levels = c("Period", "Formation"))) %>%
...
for(i in seq_along(unique.types)){
  ...
  tt <- tableGrob(d = select(period.df, name),
                  ...
                  theme = ttheme_minimal(
                    core = list(bg_params = list(fill = rgb(period.df$R,
                                                            period.df$G, 
                                                            period.df$B, 
                                                            maxColorValue=255),
                                                 col = "black"),
...
Fest_lith <- rasterGrob(img, height = unit(1,"npc"), width= unit(2,"npc"),  
                        x=unit(0,"npc"))
...
# Deltadalen
# geochronology 
#setwd(paste(path, "/data/litho", sep=""))
...
filter.BB_chron <- BB_chron %>% 
  ...
  mutate(bottom = min(bottom, max(y.range)), top = max(top, min(y.range))) %>%
  
  ungroup() %>%
  
  mutate(height = (bottom-top) / (max(bottom)- min(top))) %>%
...
# change Porg to ug/g units
all.CD.long <- all.CD.long %>% mutate(value = if_else(phase == "P_org", 
                                                      value * 10,
                                                      value)
                                      )
...
plot.2 <- all.CD.long %>% 
          filter(section == "Festningen" | section == "Deltadalen") %>% 
          filter(phase == "P_aut" | phase == "P_Fe" | phase == "P_det" |  
                 phase == "P_org") %>% 
          mutate(phase = factor(
                            case_when(
                                  phase ==  "P_aut" ~  "P[aut]",
                             ...
                             levels = c("P[aut]", "P[Fe]", "P[det]", "P[org]"), 
                             ordered = TRUE
                              )
                 )
...
plot.2.f <- plot.2 %>%
              filter(section == sec.select) %>% 
              ggplot( . , aes(y = height, x = value)) +  
              geom_blank(data = range.2 %>% filter(section == sec.select), 
                         aes(y = height, x = value))  
